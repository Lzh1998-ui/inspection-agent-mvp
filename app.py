...
# ===== AQL 标准表与判定引擎已抽取到共享库 core.aql =====
# 免费版(app_free)与付费Agent版(app_pro)共用同一份内核,改一次两边受益。
# 下方 _CORE_AQL_LEGACY 定义已被 core.aql 取代,保留占位以兼容历史引用。
from core.aql import (
    AQL_SAMPLE_SIZE_CODE, AQL_SAMPLE_SIZE, AQL_AC_RE,
    get_aql_sample_info, compute_three_layer_acre,
    judge_three_layer, format_acre_hint,
)
...
# JSON 解析辅助(clean_json_string/extract_balanced_json/extract_json_robust)已抽取到 core.ai
...
# 导入PDF生成模块
from core.pdf import generate_inspection_pdf, check_font_available
from core.ai import build_ai_client, analyze_images as core_analyze_images
# 导入用户认证模块
from auth_helper import (
    is_supabase_configured,
...
def get_ai_client():
    """读取 Streamlit secrets 并委托 core.ai.build_ai_client 构建客户端(薄封装)。"""
    try:
        ...
        return build_ai_client(qwen_key, deepseek_key, openai_key, API_TIMEOUT_SECONDS)
...
# ===== AI分析函数 =====
# _format_acre_hint / compute_three_layer_acre / judge_three_layer 已抽取到 core.aql


def analyze_product_images(uploaded_files, product_name, inspection_standard, acre=None):
    """薄封装:检查密钥可用性 + 提取图片字节,核心分析逻辑在 core.ai.analyze_images。"""
    # 1. 检查 API 可用性
    ...
    for uploaded_file in uploaded_files:
        try:
            image_bytes_list.append(uploaded_file.getvalue())
        except Exception as img_error:
            return False, f"图片处理失败:{uploaded_file.name} - {str(img_error)}"

    return core_analyze_images(
        client, model_name, image_bytes_list,
        product_name, inspection_standard, acre=acre,
...
with col_btn2:
    ...
    if st.button(
        ...
        if not uploaded_files:
            ...
            # 计算三层真实 Ac/Re（基于订单数量 + 每层 AQL）
            acre = compute_three_layer_acre(order_quantity, aql_critical, aql_major, aql_minor)
            # 调用AI分析
